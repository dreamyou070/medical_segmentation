...
class TrainDataset_Seg_Tuft(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        gt_img = self.load_image(gt_path, self.mask_res, self.mask_res, type='L')  # np.array, 256,256 (128,128)
        gt_img = np.where(gt_img > 255/2, 1, 0)
        gt_arr_ = to_categorical(gt_img, num_classes=self.n_classes)
        ...
        # [3] gt flatten
        gt_flat = gt_img.flatten() # 128*128
